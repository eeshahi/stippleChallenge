...
img_raster <- as.raster(original_img)
plot(c(0, img_width), c(0, img_height), type = "n", xlab = "", ylab = "", axes = FALSE, main = "Original Image")
...
# Convert to numeric matrix - transpose because magick uses [width, height] but we want [height, width]
...
# Original - display using rasterImage
img_raster <- as.raster(img_resized)
h <- nrow(img_matrix)
w <- ncol(img_matrix)
plot(c(0, w), c(0, h), type = "n", xlab = "", ylab = "", axes = FALSE, main = "Original Image")
rasterImage(img_raster, 0, 0, w, h)

# Importance map - display as grayscale image
image(1:w, 1:h, t(importance_map), col = gray(seq(0, 1, length.out = 256)),
      xlab = "", ylab = "", axes = FALSE, main = "Importance Map", asp = 1)

# Stipple pattern - display as grayscale image
image(1:w, 1:h, t(stipple_pattern), col = gray(seq(0, 1, length.out = 256)),
      xlab = "", ylab = "", axes = FALSE, main = "Blue Noise Stippling", asp = 1)
```
...
img_raster <- as.raster(img_resized)
plot(c(0, w), c(0, h), type = "n", xlab = "", ylab = "", axes = FALSE, main = "Original Image with Grid")
rasterImage(img_raster, 0, 0, w, h)

# Draw grid lines
for(i in 0:grid_rows) {
  y <- i * section_h
  abline(h = h - y, col = 'red', lwd = 0.5, lty = 2)  # Flip Y axis for rasterImage coordinates
}
...
# Stipple pattern - display as grayscale image
image(1:w, 1:h, t(stipple_pattern), col = gray(seq(0, 1, length.out = 256)),
      xlab = "", ylab = "", axes = FALSE, main = "Stippled Image", asp = 1)
```
