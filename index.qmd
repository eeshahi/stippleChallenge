...
img_list <- lapply(1:length(frames), function(i) {
  ...
  png(temp_file, width = ncol(frame), height = nrow(frame), bg = "white", res = 72)
...
image(1:w, 1:h, t(img_matrix), 
      col = gray(seq(0, 1, length.out = 256)),
      xlab = "", ylab = "", axes = FALSE, main = "Original Image with Grid",
      asp = h/w, ylim = c(h, 1))
...
# Stipple pattern - display as grayscale image
image(1:w, 1:h, t(stipple_pattern), col = gray(seq(0, 1, length.out = 256)),
      xlab = "", ylab = "", axes = FALSE, main = "Stippled Image", 
      asp = h/w, ylim = c(h, 1))
```
